...
void fprint_dept(Database* db, ofstream* file, const string Dept, const string dept, const string id) {
	*file << dept << ": [\n";
...
void js_compile(Database* db) {
	...
	fprint_dept(db, &file, "Math", "math", "math");
	file << "\n],\n";
	fprint_dept(db, &file, "Physics", "physics", "phys");
	file << "\n]\n";
